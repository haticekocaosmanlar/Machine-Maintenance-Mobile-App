...
public class UploadMonthly extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_upload_monthly);

        monthlyView = (ImageView) findViewById(R.id.monthlyImage);
    ...
    @SuppressLint("NewApi")
    public void selectMonthlyImage(View view) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {  //seçtik ve image'imizi alıcaz
        ...
        if (requestCode == 2 && resultCode == RESULT_OK && data != null) {  //data bizim image
            ...
            try {
                ...
                monthlyView.setImageBitmap(bitmap);  //bitmapi imageview e set etme
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    String uuidS;
    public void uploadMonthly(View view) {


        UUID uuidImage = UUID.randomUUID(); //unique id verme
        String imageName = "monthly/" + uuidImage + ".jpg";
        ...
        storageReference.putFile(selected).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {  //resim yükleme başarılı olursan ne yapılacak
                ...
                myRef.child("Monthly").child("monthlySchedule").child("downloadURL").setValue(downloadURL);
        ...
        Toast.makeText(getApplicationContext(), "Aylık Bakım Planı Eklendi", Toast.LENGTH_LONG).show();
