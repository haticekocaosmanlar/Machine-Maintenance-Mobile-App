...
public class UploadWeekly extends AppCompatActivity {

    ImageView weeklyView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_upload_weekly);

        weeklyView = (ImageView) findViewById(R.id.monthlyImage);
    ...
    @SuppressLint("NewApi")
    public void selectWeeklyImage(View view) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {  //seçtik ve image'imizi alıcaz
        ...
        if (requestCode == 2 && resultCode == RESULT_OK && data != null) {  //data bizim image
            ...
            try {
                ...
                weeklyView.setImageBitmap(bitmap);  //bitmapi imageview e set etme
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    String uuidS;
    public void uploadWeekly(View view) {


        UUID uuidImage = UUID.randomUUID(); //unique id verme
        String imageName = "weekly/" + uuidImage + ".jpg";
        ...
        storageReference.putFile(selected).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {  //resim yükleme başarılı olursan ne yapılacak
                ...
                myRef.child("Weekly").child("weeklySchedule").child("downloadURL").setValue(downloadURL);
        ...
        Toast.makeText(getApplicationContext(), "Haftalık Bakım Planı Eklendi", Toast.LENGTH_LONG).show();
